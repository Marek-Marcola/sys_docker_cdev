...
if [ $HELP -eq 1 ]; then
  ...
  echo "$(basename $0) -P  [-R repo] [-F from] [-FR reg]                       # pull, repo, from_image, from_reg"
  echo "$(basename $0) -b  [-R repo] [-V ver] [-T tags] [-t date]              # build, repo, version, tags, tag YYYYMMDDhhmm"
  ...
  echo "$(basename $0) -bf [-R repo]                                           # build from list, repo"
  echo "$(basename $0) -p  [-R repo] [-V ver] [-T tags] [-t date]              # push, repo, version, tags, tag YYYYMMDDhhmm"
  echo "$(basename $0) -r  [-R repo] [-V ver|tag] [args1] [-- args2]           # docker run prefix/repo:tag"
  echo "$(basename $0) -rr [-R repo] [-V ver|tag] [args1] [-- args2]           # docker run reg/prefix/repo:tag"
  echo "$(basename $0) -k  [-R repo] [-V ver|tag] [args1] [-- args2]           # k8s run reg/prefix/repo:tag"
  echo "$(basename $0) -e  [-R repo] [-V ver|tag] [args1]                      # docker exec in reg/prefix/repo:tag cdev-repo-xxxxx"
  echo "$(basename $0) -l  [-R repo]                                           # list prefix/repo"
  echo "$(basename $0) -lr [-R repo]                                           # list regs/prefix/repo"
  echo "$(basename $0) -dr[k] [-R repo]                                        # image delete regs/prefix/repo (default: k[eep]=$KEEPR)"
  echo "$(basename $0) -d[k]  [-R repo]                                        # image delete prefix/repo (default: k[eep]=$KEEP)"
  echo "$(basename $0) -H[w]  [-R repo]                                        # image history (default: w[idth]=$WIDTH)"
  echo "$(basename $0) -i                                                      # image inspect"
  echo "$(basename $0) -ic                                                     # image chain"
  echo "$(basename $0) -if                                                     # image files"
  echo "$(basename $0) -is dir  [-R repo] [-V ver] [-t date] [-S-suffix] [-z]  # image save, repo, version, YYYYMMDDhhmm, suffix, gzip"
  echo "$(basename $0) -il dir  [-p] [-A]                                      # image load (all from dir), push, archive"
  echo "$(basename $0) -il file [-R repo -V ver -t date [-S-suffix]] [-p] [-A] # image load, repo, version, YYYYMMDDhhmm, suffix, push, archive"
